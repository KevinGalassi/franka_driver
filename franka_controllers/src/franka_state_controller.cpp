...
#include <franka_controllers/franka_state_controller.h>
                                 ...
                                 ros::NodeHandle& controller_node_handle) 
{
    ...
    if (!controller_node_handle.getParam("publish_rate", publish_rate))
        ROS_INFO_STREAM("StatePublisher: Did not find publish_rate. Using default " << publish_rate << " [Hz].");
    trigger_publish = franka_hw::TriggerRate(publish_rate);

    if (!controller_node_handle.getParam("joint_names", joint_names_) || joint_names_.size() != robot_state_.q.size())
    {
        ROS_ERROR("StatePublisher: Invalid or no joint_names parameters provided, aborting controller init!");
        return false;
    }
    try {
    ...
    /* Wrench Estimation required */
        auto* model_interface = robot_hardware->get<franka_hw::FrankaModelInterface>();
    if (model_interface == nullptr) { ROS_ERROR_STREAM("StatePublisher: Error getting model interface from hardware");
        return false;
    }
    try {
        ...
        ROS_ERROR_STREAM( "StatePublisher: Exception getting model handle from interface: " << ex.what());
        return false;
    }
...
void FrankaStateController::starting(const ros::Time& time ) {
...
void FrankaStateController::publishExternalForce(const ros::Time& time) {
...
void FrankaStateController::publishExternalForceFilter(const ros::Time& time) {
...
PLUGINLIB_EXPORT_CLASS(franka_controllers::FrankaStateController, controller_interface::ControllerBase)
