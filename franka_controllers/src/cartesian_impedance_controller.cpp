...
bool CartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
{
   ...
   dynamic_server_compliance_param_ = std::make_unique<  dynamic_reconfigure::Server<franka_controllers::compliance_paramConfig>>(
...
void CartesianImpedanceController::complianceParamCallback(franka_controllers::compliance_paramConfig& config, uint32_t /*level*/) 
{
   /*
   ...
   */
   ...
   Eigen::Matrix<double, 3, 1> stiff_trans_param, stiff_rot_param;
   stiff_trans_param << config.trasl_x_stiffness, config.trasl_y_stiffness, config.trasl_z_stiffness;
   stiff_rot_param << config.rot_x_stiffness, config.rot_y_stiffness, config.rot_z_stiffness;

   cartesian_stiffness_target_ <<   stiff_trans_param(0),0,0,0,0,0,
                                    0,stiff_trans_param(1),0,0,0,0,
                                    0,0,stiff_trans_param(2),0,0,0,
                                    0,0,0,stiff_rot_param(0),0,0,
                                    0,0,0,0,stiff_rot_param(1),0,
                                    0,0,0,0,0,stiff_rot_param(2);

   std::cout << "new cartesian_stiffness_target_ \n" << cartesian_stiffness_target_ << "\n";

   stiff_trans_param << sqrt(config.trasl_x_stiffness), sqrt(config.trasl_y_stiffness), sqrt(config.trasl_z_stiffness);
   stiff_rot_param << sqrt(config.rot_x_stiffness), sqrt(config.rot_y_stiffness), sqrt(config.rot_z_stiffness);

   cartesian_damping_target_ <<   stiff_trans_param(0),0,0,0,0,0,
                                    0,stiff_trans_param(1),0,0,0,0,
                                    0,0,stiff_trans_param(2),0,0,0,
                                    0,0,0,stiff_rot_param(0),0,0,
                                    0,0,0,0,stiff_rot_param(1),0,
